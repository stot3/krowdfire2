...
(function (factory) {
  ...
  var server, server_loc, currentPath, parsedPath;
  // Custom end
  ...
  $.FE.PLUGINS.file = function (editor) {
    ...
    function insert (link, text, response) {
     ...
     // Custom
      // Insert the link.
      editor.html.insert('<a href="' + link + '" id="fr-inserted-file" class="fr-file" target="_blank">' + (text || editor.selection.text()) + '</a>');
      // Custom ends
        ...
        if (editor.opts.fileUploadToS3) {
          ...
          if (status >= 200 && status < 300) {
            ...
            if (resp) {
              // Custom
              var fileURL;
              if (parsedPath == "admin" || parsedPath == "campaign-setup" || parsedPath == "campaign-manager"){
              ...
              else if (parsedPath == "rewards"){
                fileURL = server + "/image/campaign_reward_image/" + resp.path_external;
              }
              insert(fileURL, text, response || responseText);
              // Custom End
            }
          }
          else {
    ...
    function upload (files) {
      ...
      if (typeof files != 'undefined' && files.length > 0) {
        ...
        if (form_data) {
          ...
          // Custom code for uploading files into our own endpoint
          ...
          if (parsedPath == "admin") {
            ...
            data.append('resource_content_type', 'generic');
            ...
            if (parsedPath == "campaign-manager"){
              campaignID = currentPath.split("/")[3];
            }
            else {
              campaignID = currentPath.split("/")[2];
            }
            server = $(".page-content").attr("data-server");
            server_loc = $(".page-content").attr("data-loc");
